...
*   **一次性会话密钥 (`ONETIME_KEY`)**:
    ...
    *   **安全性与性能权衡**: `ONETIME_KEY` 的传输依赖于 `HTTPS` 提供的信道安全。为了在高并发场景下获得更高性能，密钥在使用后不会立即从缓存中删除，而是依赖预设的 TTL (Time-To-Live) 自动过期。这是一种典型的安全与性能的权衡：它降低了每次请求的写操作开销，但代价是在密钥的生命周期内（例如 60 秒），存在理论上的重放攻击风险。
...
*   **密钥管理**:
    *   主密钥从不离开服务器。
    *   一次性密钥通过较短的 TTL (Time-To-Live) 自动过期，以此在性能和安全性之间取得平衡。
