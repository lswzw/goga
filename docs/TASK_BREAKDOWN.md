...
- [x] **2.2. 创建并服务化加密脚本文件**
  - **描述**:
    1.  在项目下创建一个 `static` 或类似目录，并放入一个空的 `goga.js` 文件。
    2.  在网关的路由逻辑中添加一个处理器，用于响应 `GET /goga.js` 请求，并返回该文件的内容。
  - **优先级**: 高。
  - **依赖**: 2.1
  - **产出**: 浏览器可以通过网关正确下载 `goga.js` 文件，避免 404 错误。
...
**目标**: 完成客户端 `goga.js` 的逻辑，使其能够与服务端协同完成整个加密流程。

- [x] **4.1. 编写客户端加密脚本**
  - **描述**: 填充 `goga.js` 文件的内容，实现设计文档中描述的全部逻辑：
    1.  监听并阻止表单默认提交。
...
- [x] **6.1. 在 goga.js 中引入客户端密钥缓存**
  - **描述**: 在 `goga.js` 脚本的闭包（IIFE）中，定义一个用于存储加密密钥、令牌和过期时间戳的内存变量。此缓存将随页面刷新而自动清除。
  - **优先级**: 高。
  - **依赖**: Phase 4 完成。
  - **产出**: 脚本内部具备了存储密钥的缓存结构。
...
- [x] **6.4. 协调客户端与服务端的缓存超时策略**
  - **描述**: 在 `goga.js` 中为客户端缓存设置一个明确的超时时间（例如4分钟）。此时间必须小于 Go 后端 `keycache` 模块中为密钥设置的服务器端 TTL（例如5分钟），以避免客户端使用一个已被服务端丢弃的密钥。
  - **优先级**: 中。
  - **依赖**: 6.1, 3.2
...
- [X] **10.6. 【中优】实现“按字段加密”** (已标记为“改动过大，暂不处理”类别)
  - **描述**: 客户端脚本应支持可配置地对某些特定表单字段进行加密，而不是对整个表单进行加密。当前 `goga.js` 将整个表单序列化为 JSON 并加密，不满足部分字段加密的需求。
  - **优先级**: 中。
  - **依赖**: Phase 4 完成，客户端脚本需修改。
  - **产出**: 一个允许用户配置哪些表单字段需要加密的客户端脚本。

- [x] **10.7. 【健壮性】客户端动态配置密钥缓存有效期**
  - **描述**: 修改 `static/goga.js` 脚本。密钥分发接口 `/goga/api/v1/key` 应返回一个 `ttl` (Time-To-Live) 值，指示密钥的有效时长。客户端脚本应使用此 `ttl` 值（例如，`ttl * 0.8` 作为安全余量）来动态计算和设置本地密钥缓存的有效期，而不是使用硬编码的4分钟。这消除了前后端配置的强耦合，提升了系统的健壮性和灵活性。
  - **优先级**: 高。
...
*   [x] **[任务 4] 实现安全性和边缘案例测试**
    ...
    *   创建一个测试，以确保 `/goga.min.js` 资产被正确提供 (`TestStaticAssetDelivery`)。
...
**背景**: 当前客户端加密脚本 `static/goga.js` 的核心安全逻辑依赖于一个在前端代码中定义的 `excludeUrls` 白名单。此设计存在严重的安全风险，因为它错误地假设客户端环境是可信的，而任何在浏览器中运行的代码都可能被最终用户拦截、查看和修改。为应对此威胁，必须采用“纵深防御”策略，其核心原则是“永不信任客户端”，安全策略的最终决策权必须掌握在服务器端。
...
- [x] **15.2. 【高优】实现子资源完整性 (SRI)**
  - **描述**: 为 `goga.js` 脚本的加载启用子资源完整性 (SRI) 保护，以防御中间人攻击。在构建流程中为脚本文件生成 SHA-384 哈希，并在 `proxy.go` 的脚本注入逻辑中，将该哈希值添加到 `<script>` 标签的 `integrity` 属性里，同时添加 `crossorigin="anonymous"` 属性。
  - **优先级**: 高。
  - **依赖**: 2.1 (脚本注入逻辑)。
  - **产出**: 一个能抵抗传输层内容篡改的脚本加载机制。如果脚本在网络中被修改，浏览器将直接拒绝执行它。

- [X] **15.3. 【高优】实现客户端运行时自我完整性校验** (已标记为“暂不处理”类别)
  - **描述**: 为 `goga.js` 增加运行时自我完整性校验，以增加客户端篡改（如修改 `excludeUrls` 白名单）的难度。在构建阶段，计算关键配置的哈希指纹并将其与配置一同嵌入脚本。在脚本运行时，立即重新计算哈希并与嵌入的指纹比对。若不匹配，则立即执行“安全锁定”，通过覆盖 `fetch` 等全局函数来瘫痪所有网络请求。
  - **优先级**: 高。
  - **依赖**: Phase 4 (客户端加密实现)。
...
P16 中的任务大部分都非常重要，P0级别的三个任务（16.1, 16.2, 16.3）确实是当前脚本的核心功能缺陷。然而，它们对代码改动的大小和性能的影响差异巨大。其中，**自动解密 (16.1)** 和 **支持非字符串请求体 (16.3)** 这两项，代码改动会非常大，并且是主要的性能风险点，在高并发（1000 QPS）目标下需要重点关注。

**最终决策**:

根据分析，并考虑到当前版本的稳定性与性能目标，决定如下：
- **暂不处理 16.1，16.2, 16.3**: 鉴于其巨大的改动和性能风险，这两个任务在此版本中将不予实现。未来若有需求，可考虑在独立的分支上作为增强版本进行开发。
- **优先处理 16.4, 16.5**: 这些任务成本低、收益高，将按计划完成。

---

##### 详细分析摘要

| 任务 | 重要性 | 代码改动复杂度 | 对 1000 QPS 性能目标的影响 |
| :--- | :--- | :--- | :--- |
| **16.1 响应解密** | 极高 (功能完整性) | **巨大** | **高风险** (网关CPU开销翻倍) |
| **16.2 支持PUT/PATCH** | 高 (功能完整性) | 小 | 无直接影响 |
| **16.3 支持FormData/Blob**| 高 (功能完整性) | **巨大且复杂** | **极高风险** (网关内存和CPU开销剧增) |
| **16.4 Fail-Closed** | 高 (安全性) | 小 | 无 |
| **16.5 URL配置化** | 低 (可维护性) | 非常小 | 无 |

---

**目标**: 解决 `goga.js` 在功能、安全和可维护性方面的不足，提升其健壮性和适用范围。

- [X] **16.1. [P0] 实现响应体自动解密 (已标记为“决策：暂不处理”)**
...
- [X] **16.2. [P0] 扩展支持 `PUT` 和 `PATCH` 请求方法 (已标记为“决策：暂不处理”)**
  - **描述**: 修改请求拦截器中的方法判断逻辑，将加密范围从仅 `POST` 扩展到 `POST`, `PUT`, `PATCH` 等所有可能携带请求体的方法。
  - **优先级**: P0 - 严重功能缺陷。
  - **依赖**: Phase 4 完成。

- [X] **16.3. [P0] 支持 `FormData`, `Blob` 等非字符串请求体 (已标记为“决策：暂不处理”)**
...
- [X] **16.4. [P1] 实现可配置的“失败时阻断/放行”策略 (服务端控制) (已标记为“决策：暂不处理”)**
  - **描述**: 扩展 GoGa 网关后端和客户端脚本，实现一个由服务端统一控制的“失败时阻断/放行”（Fail-Closed/Fail-Open）策略，以增强安全性与运维灵活性。
    1.  **后端 (`configs/config.yaml` & `/goga/api/v1/key`):**
        *   在配置中增加 `encryption.fail_closed` (布尔类型，默认为 `true`，即默认阻断)。
        *   修改 `GET /goga/api/v1/key` 接口，使其在返回 `key`, `token`, `ttl` 的同时，也包含 `fail_closed` 标志。
    2.  **前端 (`goga.js`):**
        *   客户端脚本应从 `/goga/api/v1/key` 接口的响应中获取 `fail_closed` 标志，并与密钥一起缓存。
        *   在加密过程失败时，脚本应检查缓存的 `fail_closed` 标志：
            *   如果为 `true`，则中断网络请求并抛出错误 (阻断)。
            *   如果为 `false`，则发送原始明文请求 (放行)。
        *   **安全默认**: 在客户端首次加载，还未成功获取到密钥和配置之前的“冷启动”阶段，脚本应默认执行**阻断**行为，确保安全。
  - **优先级**: P1 - 中等安全风险 (策略控制)。
  - **依赖**: Phase 4 完成，后端 `/goga/api/v1/key` 接口需扩展。

- [X] **16.5. [P2] 将密钥获取URL配置化 (已标记为“决策：暂不处理”)**
...
### Phase 17: 安全与功能性增强（2025-11-26 新增）

**目标**: 进一步提升网关的错误处理一致性、日志结构化、输入安全和接口鉴权能力。

- [x] **17.1. 统一错误处理与响应格式**

  - **描述**:
    1. 梳理所有模块（加密、网关、缓存、中间件、配置等）的错误返回和日志，统一错误码和响应格式。
    2. 明确哪些错误需返回客户端（如鉴权失败、参数错误），哪些仅记录日志（如内部异常、敏感信息）。
    3. 增加全局错误处理和 panic 恢复测试，确保所有异常都能被捕获并安全处理。
  - **优先级**: 高。
  - **依赖**: Phase 1-16。
  - **产出**: 一个错误处理一致、响应格式统一、异常可控的网关系统。

- [x] **17.2. 日志结构化与监控能力增强**
  - **描述**:
    1. 梳理并结构化所有日志内容，确保关键操作、异常和安全事件均有明确记录。
    2. 增加 traceId/requestId 支持，便于分布式链路追踪和故障定位。
    3. 把 msg="http request"无效日志修改成展示hosts。
    3. 为后续监控系统（如 Prometheus、ELK）集成预留接口。
  - **优先级**: 高。
  - **依赖**: 7.3, 12.1。
  - **产出**: 一个日志结构化、可追踪、易于监控的网关系统。

- [ ] **17.3. 外部接口速率限制**
  - **描述**:
    1. 梳理所有外部 API，尤其是敏感接口（如 `/goga/api/v1/key`）。
    2. 增加速率限制（Rate Limiting），防止接口被滥用或暴力攻击。
  - **优先级**: 高。
  - **依赖**: Phase 3.3, 3.4, 15.1。
  - **产出**: 一个具备速率限制能力的安全 API 网关。

---
### Phase 18: 代理性能优化 (目标: 1000 QPS)

**目标**: 将代理服务重构为“流式处理”架构，以大幅降低内存分配和GC压力，为实现1000 QPS的性能目标奠定基础。

- [x] **18.1. 实现流式脚本注入器 (`scriptInjector`)**
  - **描述**: 创建一个自定义的 `io.Reader` 实现，命名为 `scriptInjector`。它将包装上游 Reader，通过内部缓冲和搜索机制，在不将整个响应读入内存的前提下，流式地查找 `</body>` 标签并注入脚本。
  - **优先级**: 最高 (后续任务的基础)。
  - **依赖**: 无。
  - **产出**: 一个可独立测试、高效且内存占用低的 `scriptInjector.go` 模块。

- [x] **18.2. 重构 `ModifyResponse` 以支持流式处理**
  - **描述**: 重构 `internal/gateway/proxy.go` 中的 `ModifyResponse` 函数。当需要注入脚本时，删除 `Content-Length` 头并设置 `Transfer-Encoding: chunked`。使用 `io.LimitedReader` 包装原始响应体以实现大小限制。
  - **优先级**: 高。
  - **依赖**: Phase 2 (原始注入逻辑)。
  - **产出**: 一个为接入流式处理单元做好准备的 `ModifyResponse` 函数框架。

- [x] **18.3. 集成注入器并构建流式处理链**
  - **描述**: 将 `scriptInjector` 集成到 `ModifyResponse` 中。对于无压缩响应，直接将 `resp.Body` 替换为 `scriptInjector`。对于压缩响应，使用 `io.Pipe` 创建管道，在新的 goroutine 中构建完整的流式处理链：`原始 Body -> 解压 Reader -> 注入 Reader -> 压缩 Writer -> PipeWriter`，并将 `PipeReader` 设为最终的 `resp.Body`。
  - **优先级**: 高。
  - **依赖**: 18.1, 18.2。
  - **产出**: 一个功能完整的、采用流式架构处理响应的代理。

- [x] **18.4. 引入内存池 (`sync.Pool`) 优化**
  - **描述**: 为流式处理链中的所有临时缓冲区（特别是在 `scriptInjector` 和 `io.Copy` 中）引入 `sync.Pool` 进行复用。确保所有 `klauspost/compress` 库的调用都已配置为使用内存池模式。
  - **优先级**: 中。
  - **依赖**: 18.3。
  - **产出**: 一个内存分配效率更高、GC压力更小的代理服务。

---

### Phase 19: 解密中间件流式优化

**目标**: 将解密中间件重构为流式处理架构，消除大内存分配，与 Phase 18 的性能优化目标保持一致，确保解密流程不会成为性能瓶颈。

- [x] **19.1. 实现流式解密检测器**
  - **描述**: 创建一个轻量级的请求体预读取机制，能够在不完整读取整个请求体的情况下，快速判断是否为加密载荷。实现一个 `peekReader` 来读取前 N 个字节进行格式检测。
  - **优先级**: 最高 (后续任务的基础)。
  - **依赖**: Phase 18 完成。
  - **产出**: 一个高效的加密请求检测器，避免不必要的完整请求体读取。

- [x] **19.2. 实现流式解密 Reader**
  - **描述**: 创建一个自定义的 `io.Reader` 实现 `decryptReader`，它将包装原始请求体，在流式读取过程中实时解密数据。支持分块解密，避免将整个加密载荷加载到内存。
  - **优先级**: 高。
  - **依赖**: 19.1。
  - **产出**: 一个支持流式解密的 Reader 包装器。

- [x] **19.3. 引入解密缓冲区内存池**
  - **描述**: 为解密过程中的临时缓冲区引入 `sync.Pool` 内存池。包括请求体读取缓冲区、Base64 解码缓冲区、AES 解密缓冲区等。确保所有缓冲区都能被高效复用。
  - **优先级**: 高。
  - **依赖**: 19.2。
  - **产出**: 一套完整的解密缓冲区内存池系统，显著减少内存分配。

- [x] **19.4. 重构 DecryptionMiddleware 支持流式处理**
  - **描述**: 重构 `DecryptionMiddleware`，使用新的流式检测和解密组件。对于加密请求，将 `r.Body` 替换为 `decryptReader`；对于普通请求，保持原始 Body 不变。确保与现有的密钥缓存系统完全兼容。
  - **优先级**: 高。
  - **依赖**: 19.1, 19.2, 19.3。
  - **产出**: 一个完全流式化的解密中间件，内存使用效率大幅提升。

- [x] **19.5. 添加解密性能监控和指标**
  - **描述**: 在解密流程中添加性能监控点，记录解密耗时、内存使用量、处理吞吐量等关键指标。为后续的性能调优提供数据支持。
  - **优先级**: 中。
  - **依赖**: 19.4。
  - **产出**: 完善的解密性能监控体系，便于识别性能瓶颈。